...
if (exists("huc_level")) plot(results_shape, col = NA, axes = TRUE) else
...
plot(spTransform(referenceBoundaries[!referenceBoundaries$STATE_NAME %in% studyAreaExtent$STATE_NAME,], CRSobj = results_shape@proj4string), add=T, border="gray50", lwd=0.3) #, col = "white"
plot(spTransform( studyAreaExtent, CRSobj = results_shape@proj4string), add=T, border="black", lwd=0.7)
legend("topleft", legend=names(attr(colcode, "table")), col=attr(colcode, "palette"), cex=0.8, bty="o", lwd = 4, bg = "white")
box()
@
